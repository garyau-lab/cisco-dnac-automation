...
def pre_check(mgmt_ip, interface_name, port):
    try:
        ...
        if response['status'] == "success" and response['dataIpAddressPoolName'] == port['data_pool'] and response['scalableGroupName'] == port['sgt_name'] and response['voiceIpAddressPoolName'] == port['voice_pool'] and response['authenticateTemplateName'] == port['auth_template']:
            return ("same")
...
def creation(mgmt_ip, interface_name, port):
    payload = {
        ...
        "scalableGroupName": port['sgt_name'],
        "voiceIpAddressPoolName": port['voice_pool'],
        "authenticateTemplateName": port['auth_template']
    }
    dna.sda.add_port_assignment_for_user_device(payload=payload)
...
print_header('Perform port assigment for USER')
for port in port_dict:
    if port['port_type'] == "user":
        mgmt_ip = get_mgmt_ip(port)
        intf_names = construct_intf_list(port)

        # Start
        for i in intf_names:
            port_detail = port['data_pool']
            if port['sgt_name'] != "":
                port_detail= port_detail+" "+port['sgt_name']
            if port['voice_pool'] != "":
                port_detail= port_detail+" "+port['voice_pool']                
            print_action(i+" ["+port_detail+"]")
